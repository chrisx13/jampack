...
/** Filtre société : société active, ou {} en vue consolidée. */
...
export const crmRouter = router({
  ...
  companies: router({
    ...
    create: authed('create', 'Company').input(companyCreate).mutation(({ ctx, input }) => {
      ...
      return withTenant(ctx.user.organizationId, (tx) =>
    ...
    update: authed('update', 'Company').input(companyUpdate).mutation(({ ctx, input }) => {
      const { id, ...data } = input;
      return withTenant(ctx.user.organizationId, (tx) => tx.company.update({ where: { id }, data }));
    }),
  ...
  establishments: router({
    ...
    update: authed('update', 'Company').input(establishmentUpdate).mutation(({ ctx, input }) => {
      const { id, ...data } = input;
      return withTenant(ctx.user.organizationId, (tx) => tx.establishment.update({ where: { id }, data }));
    }),
  ...
  contacts: router({
    ...
    create: authed('create', 'Contact').input(contactCreate).mutation(({ ctx, input }) => {
      const societeId = requireSociete(ctx.societeId);
      return withTenant(ctx.user.organizationId, (tx) => tx.contact.create({ data: { ...input, organizationId: ctx.user.organizationId, societeId } }));
    }),
    update: authed('update', 'Contact').input(contactUpdate).mutation(({ ctx, input }) => {
      const { id, ...data } = input;
      return withTenant(ctx.user.organizationId, (tx) => tx.contact.update({ where: { id }, data }));
    }),
  ...
  opportunities: router({
    ...
    create: authed('create', 'Opportunity').input(opportunityCreate).mutation(({ ctx, input }) => {
      const societeId = requireSociete(ctx.societeId);
      return withTenant(ctx.user.organizationId, (tx) => tx.opportunity.create({ data: { ...input, organizationId: ctx.user.organizationId, societeId } }));
    }),
    update: authed('update', 'Opportunity').input(opportunityUpdate).mutation(({ ctx, input }) => {
      const { id, ...data } = input;
      return withTenant(ctx.user.organizationId, (tx) => tx.opportunity.update({ where: { id }, data }));
    }),
    /** Déplacement dans le pipeline (kanban). */
    move: authed('update', 'Opportunity').input(opportunityMove).mutation(({ ctx, input }) =>
      withTenant(ctx.user.organizationId, (tx) => tx.opportunity.update({ where: { id: input.id }, data: { stageId: input.stageId } }))
    ),
  ...
  stages: router({
    list: protectedProcedure.query(({ ctx }) =>
      withTenant(ctx.user.organizationId, (tx) => tx.pipelineStage.findMany({ orderBy: { order: 'asc' } }))
    ),
  }),
