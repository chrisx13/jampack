...
export const accountingRouter = router({
  ...
  /** Export CSV du grand livre d'un compte (Date ; Journal ; Réf. ; Libellé ; Débit ; Crédit ; Lettrage ; Solde). */
  exportLedger: authed('read', 'Accounting').input(z.object({ accountId: z.string().min(1) })).query(({ ctx, input }) =>
    withTenant(ctx.user.organizationId, ctx.societeId, async (tx) => {
      const account = await tx.account.findFirstOrThrow({ where: { id: input.accountId, ...scope(ctx.societeId) }, select: { code: true, name: true } });
      const lines = await tx.journalEntryLine.findMany({
        where: { accountId: input.accountId, entry: { is: scope(ctx.societeId) } },
        include: { entry: { select: { date: true, label: true, reference: true, journal: { select: { code: true } } } } },
        orderBy: [{ entry: { date: 'asc' } }, { position: 'asc' }],
        take: 5000,
      });
      ...
      return { filename: `grand-livre-${account.code}.csv`, content: ledgerCsv(rows) };
    })
  ),
