...
export const catalogRouter = router({
  ...
  products: router({
    ...
    create: authed('create', 'Product').input(productCreate).mutation(({ ctx, input }) => {
      ...
      return withTenant(ctx.user.organizationId, (tx) => tx.product.create({ data: { ...input, organizationId: ctx.user.organizationId, societeId } }));
    }),
    update: authed('update', 'Product').input(productUpdate).mutation(({ ctx, input }) => {
      const { id, ...data } = input;
      return withTenant(ctx.user.organizationId, (tx) => tx.product.update({ where: { id }, data }));
    }),
    remove: authed('delete', 'Product').input(byId).mutation(({ ctx, input }) =>
      withTenant(ctx.user.organizationId, (tx) => tx.product.delete({ where: { id: input.id } }))
    ),
  }),
  ...
  taxRates: router({
    list: protectedProcedure.query(({ ctx }) =>
      withTenant(ctx.user.organizationId, (tx) => tx.taxRate.findMany({ orderBy: { rate: 'desc' } }))
    ),
    create: authed('manage', 'all').input(taxRateCreate).mutation(({ ctx, input }) =>
      withTenant(ctx.user.organizationId, (tx) => tx.taxRate.create({ data: { ...input, organizationId: ctx.user.organizationId } }))
    ),
    update: authed('manage', 'all').input(taxRateUpdate).mutation(({ ctx, input }) => {
      const { id, ...data } = input;
      return withTenant(ctx.user.organizationId, (tx) => tx.taxRate.update({ where: { id }, data }));
    }),
    remove: authed('manage', 'all').input(byId).mutation(({ ctx, input }) =>
      withTenant(ctx.user.organizationId, (tx) => tx.taxRate.delete({ where: { id: input.id } }))
    ),
  }),
  ...
  sequences: router({
    list: protectedProcedure.query(({ ctx }) =>
      withTenant(ctx.user.organizationId, (tx) => tx.numberSequence.findMany({ where: scope(ctx.societeId), orderBy: { docType: 'asc' } }))
    ),
  }),
});
