...
- `Invoice` (**`docType` = devis|facture|avoir**, `status`, `source` self-relation = filiation,
  `factor`/`bankAccount`/`paymentTerm`) · `InvoiceLine` (qty/PU/TVA figés) · `Payment` (rattaché facture).
...
- `PurchaseOrder` (supplier=`Company`, `warehouse`, `status`, `number` seq « commande ») ·
  `PurchaseOrderLine` (`quantityReceived`) · `SupplierInvoice` (+ lien `purchaseOrderId`) · `SupplierInvoiceLine`.
...
**sales_documents** (docType) → **payments** → **stock** → **purchases** → **supplier_invoices**.
...
Invoice 1─* InvoiceLine   Invoice 1─* Payment   Invoice 0─1 Invoice(source)
PurchaseOrder 1─* PurchaseOrderLine   PurchaseOrder 1─* SupplierInvoice
