...
| FR-TRE-2 | Rapprochement bancaire (relevé ↔ écritures 512). | C | ✅ (voir FR-CPT-5d ; import relevé ⏳) |
...
- **REG-3 (M)** **RGPD / CNIL** : registre des traitements, minimisation, hébergement UE, droits des personnes. 🔧 (**tous les droits outillés** : accès/portabilité export, rectification, opposition, effacement anonymisant, limitation ✅ ; reste purge automatique à échéance) — voir [Sécurité & RGPD](SECURITE-RGPD.md).
