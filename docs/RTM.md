...
| FR-VEN-19 | `timeEntryAmountHt`/`formatDuration`/`timeEntryCreate` (domain), `TimeEntry` (+ RLS org/société), `time.router` (`list`/`create`/`update`/`remove`/`invoiceForCompany`), `TimeTracking.tsx` | unit : montant = heures×taux, durée lisible ; e2e int : 2 temps → facture (150 HT), temps marqués facturés + liés | ✅ |
| FR-VEN-20 | `Invoice.publicToken`/`acceptedAt`/`acceptedByName`/`acceptedIp`, `withPublicToken` + policies RLS `public_quote_token`/`public_quote_societe`/`public_quote_company`, `quotes.publicLink` (authed), `publicQuote.router` (`get`/`accept`/`decline`, publicProcedure), `PublicQuote.tsx` (route pré-auth `/devis/:token`) | e2e int : lien → lecture par jeton (société lisible), acceptation ET refus (statut/signataire/date), jeton inconnu rejeté, double réponse refusée | ✅ |
| FR-VEN-21 | `Invoice.paymentUrl`, `invoiceCreate`/`invoiceUpdate` (URL validée), affichage `invoiceHtml.ts`, champ `SalesDocs.tsx` | e2e int : lien enregistré puis effacé (null) | ✅ |
...
| Ergonomie (lot) | Recherche/filtres sur listes (compta, stock, contacts, audit ; **filtre par journal** sur écritures), **toasts** (`Toast.tsx`, remplace `alert()`), modale d'acompte (remplace `window.prompt`), confirmations de suppression | validation via CI Docker (typecheck/lint/tests/build) ; plus aucun `alert()`/`prompt()` natif | ✅ |
| Exports CSV (lot) | `aiCreditsCsv`/`opsHistoryCsv` (domaine + tests) + endpoints `documents.creditsCsv`/`ops.historyCsv` ; échéanciers clients/fournisseurs, valorisation stock (client-side) | unit (schemas.test) : en-tête/format FR/échappement | ✅ |
| Accessibilité (RGAA) | `aria-label` sur boutons à icône seule (WCAG 4.1.2) + champs de recherche ; `DECLARATION-ACCESSIBILITE.md` | — | ✅ |
| Sécurité (lot) | `eslint-plugin-security` (SAST offline, ReDoS) ; `pnpm.overrides` (audit 27→14) ; secrets chiffrés ; isolation absolue ; CI Docker | lint SAST + CI | ✅ |
